...
<head>
    ...
    <title>Records Section — All Incoming Documents</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .stat-card{background:#fff;border-radius:12px;padding:18px 20px;border:1px solid var(--border);position:relative;overflow:hidden}
        ...
        .stat-card.c-slate::before{background:#94a3b8}
        .stat-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.8px;color:#94a3b8;margin-bottom:6px}
        ...
        @media(max-width:900px){
            ...
            .table-head{padding:14px 16px;flex-direction:column;align-items:stretch;gap:10px}
            .filters{gap:6px}
            .filters input{font-size:11px;padding:6px 8px 6px 28px}
            .filters input::placeholder{font-size:10px}
            .filters select{font-size:11px;padding:6px 24px 6px 8px;min-width:100px}
            /* Hide Type(3), Sender(4), Current Office(5), Submitted(7) columns on mobile */
            th:nth-child(3),td:nth-child(3),
            th:nth-child(4),td:nth-child(4),
            th:nth-child(5),td:nth-child(5),
            th:nth-child(7),td:nth-child(7){display:none}
            /* Hide View button, keep row clickable */
...
<div class="main">
    <div class="page-header">
        <div class="page-header-top">
            <div>
                <h1>All Incoming Documents</h1>
                <p>View and monitor every document that enters DOCTRAX &mdash; <?php echo e($roleName); ?> Access</p>
            </div>
    ...
    <div class="archive-warning">
        ...
        <div><strong><?php echo e($stats['archived']); ?></strong> document(s) have been auto-archived after 7 days without being received. Use status filter "Archived" to review them.</div>
    </div>
    <?php endif; ?>
    ...
    <div class="stats-grid">
        <div class="stat-card c-blue">
            ...
            <div class="stat-num" id="stat-total"><?php echo e($stats['total']); ?></div>
            <div class="stat-sub">All time</div>
        </div>
        <div class="stat-card c-amber">
            <div class="stat-label">Awaiting Receipt</div>
            <div class="stat-num" id="stat-submitted"><?php echo e($stats['submitted']); ?></div>
            <div class="stat-sub">Submitted</div>
        </div>
        <div class="stat-card c-green">
            <div class="stat-label">Processing</div>
            <div class="stat-num" id="stat-received"><?php echo e($stats['received']); ?></div>
        ...
        <div class="stat-card c-slate">
            ...
            <div class="stat-num" id="stat-archived"><?php echo e($stats['archived']); ?></div>
    ...
    <div class="table-card">
        <div class="table-head">
            ...
            <form method="GET" action="/records/documents" class="filters" id="filterForm">
                <div class="search-wrap">
                    <i class="fas fa-search"></i>
                    <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Search reference, subject, sender...">
                </div>
                    ...
                    <tr class="doc-row" onclick='openDocDetail(<?php echo json_encode($doc->tracking_number, 15, 512) ?>)'>
                        ...
                        <td style="font-size:12px">
                            <?php if($doc->status === 'submitted'): ?>
                                <span style="color:#d97706;font-style:italic">Awaiting receipt</span>
                            <?php elseif($doc->status === 'archived'): ?>
                                <span style="color:#9ca3af;font-style:italic">Unprocessed</span>
                            <?php else: ?>
                                <?php echo e($doc->currentOffice->name ?? $doc->submittedToOffice->name ?? '-'); ?>
                            ...
                            <?php if($doc->status === 'submitted' && $doc->created_at->diffInDays(now()) >= 5): ?>
                                <div style="font-size:10px;color:#dc2626;margin-top:3px">
                                    ...
                                    <?php if($doc->created_at->diffInDays(now()) >= 7): ?>
                                        Will archive soon
                                    <?php else: ?>
                                        <?php echo e(7 - $doc->created_at->diffInDays(now())); ?> day(s) left
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="td-action">
                            <button class="btn-view" onclick="event.stopPropagation(); openDocDetail('<?php echo e($doc->tracking_number); ?>')">
    ...
    function refreshStats(){
        fetch('/api/records-stats',{headers:{'Accept':'application/json'}})
            ...
            .then(function(d){
                ...
                document.getElementById('stat-submitted').textContent = d.submitted;
                document.getElementById('stat-received').textContent  = d.received;
                document.getElementById('stat-completed').textContent = d.completed;
                document.getElementById('stat-archived').textContent  = d.archived;
